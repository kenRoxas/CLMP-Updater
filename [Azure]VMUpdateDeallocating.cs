...
{
    public class VMUpdateAzureDeallocating
    {
        [FunctionName("VMUpdateAzureDeallocating")]
        public static async Task Run([TimerTrigger("%CRON_TIME%")]TimerInfo myTimer, ILogger log)
        {
            try
            {
                log.LogInformation($"Start VMUpdateAzureDeallocating");
                ...
                var data = _db.MachineLabs.Where(q => q.IsStarted == 2).Join(_db.CloudLabUsers, a => a.UserId, b => b.UserId, (a, b) => new { a, b })
                ...
                foreach (var item in data)
                {
                    ...
                    {
                        if (item.ml.MachineStatus == "Sched Stopping")
                        {
                            UpdateMachineLab(item.ml.ResourceId, 0, 0, "", "Deallocated");
                        }
                        else
                        {
                            await Task.Run(() =>
