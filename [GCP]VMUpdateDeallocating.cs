...
{
    public static class VMUpdateDeallocatingGCP
    {
        public static string GCP = GetEnvironmentVariable("GCP");

        [FunctionName("VMUpdateDeallocatingGCP")]
        public static async Task Run([TimerTrigger("0 * * * * *")] TimerInfo myTimer, ILogger log)
        {
            try
            {
                log.LogInformation($"Start VMUpdateDeallocatingGCP");
                ...
                foreach (var item in data)
                {
                    await Task.Run(() =>
                    {
                        ...
                        if (data.data.status.ToLower() == "deallocated")
                            UpdateMachineGCP(ml, log, "SHUTDOWN", data, tenants);
