...
{
    public static class VMUpdateTimeHeartbeat
    {

        public static string AWSVM = GetEnvironmentVariable("AWSVM");
        public static string MacOs = GetEnvironmentVariable("MacOs");

        [FunctionName("VMUpdateTimeHeartbeat")]
        public static async Task Run5([TimerTrigger("0 * * * * *")] TimerInfo myTimer, ILogger log)
        //public static async Task Run5([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequest req, ILogger log)
        ...
        //public static async Task Run5([HttpTrigger(AuthorizationLevel.Function, "get", "post", Route = null)] HttpRequest req, ILogger log)
        {
            ...
            try
            {
                ...
                foreach (var item in machineUsers)
                {
                    ...
                    if (responseGetDetailsHB.Content.ReadAsStringAsync().Result != "[]")
                    {
                        ...
                        var cs = _db.CloudLabsSchedules.Where(q => q.MachineLabsId == item.MachineLabsId).FirstOrDefault();
                        ...
                        if (cs.TimeRemaining <= 0)
                        {
                            ...
                            if(item.IsStarted != 2 && item.IsStarted != 0)
                            {
                                ...
                                var v = _db.MachineLabs.Where(q => q.ResourceId == item.ResourceId).FirstOrDefault();
                                ...
                                mls.Logs = "(Deallocating)" + DateTime.UtcNow + "---" + mls.Logs;
